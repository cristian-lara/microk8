...
| 2 | Install CloudNativePG operator | Operator pods running in `cnpg-system` | From repo root: `./docs/k8s/postgres/install-cnpg-operator.sh`. Or manually: `microk8s helm3 repo add cnpg https://cloudnative-pg.github.io/charts && microk8s helm3 repo update && microk8s helm3 install cnpg cnpg/cloudnative-pg -n cnpg-system --create-namespace`. Verify: `microk8s kubectl get pods -n cnpg-system`. Alternative: addon (see `docs/08-notas-implementacion.md` §9). |
